...
const INPUT_KEYBINDINGS: &[Keybinding] = &[
    Keybinding { key: "Esc", description: "Back to home" },
    Keybinding { key: "?", description: "Toggle help" },
    Keybinding { key: "Enter", description: "Start load test" },
];

const STARTING_KEYBINDINGS: &[Keybinding] = &[
    Keybinding { key: "Esc", description: "Back to home" },
    Keybinding { key: "?", description: "Toggle help" },
];
...
enum Mode {
    Input(TextInput),
    Starting { config_path: PathBuf },
    Dashboard { table_state: TableState, auto_scroll: bool },
}
...
impl LoadTestView {
    pub const fn new() -> Self {
        Self { mode: Mode::Input(TextInput::new()) }
    }

    fn new_dashboard() -> Mode {
        let mut table_state = TableState::default();
        table_state.select(Some(0));
        Mode::Dashboard { table_state, auto_scroll: true }
    }
}
...
impl View for LoadTestView {
    fn keybindings(&self) -> &'static [Keybinding] {
        match &self.mode {
            Mode::Input(_) => INPUT_KEYBINDINGS,
            Mode::Starting { .. } => STARTING_KEYBINDINGS,
            Mode::Dashboard { .. } => DASHBOARD_KEYBINDINGS,
        }
    }
    ...
    fn tick(&mut self, resources: &mut Resources) -> Action {
        match &mut self.mode {
            Mode::Input(input) => {
                // If a loadtest is already active, jump to dashboard
                if resources.loadtest.is_some() {
                    self.mode = Self::new_dashboard();
                    return Action::None;
                }
                // If setup is in progress, transition to Starting
                if let Some(LoadTestSetup::Starting { config_path, .. }) = &resources.loadtest_setup
                {
                    self.mode = Mode::Starting { config_path: config_path.clone() };
                    return Action::None;
                }
                // If setup failed, show error in input
                if let Some(LoadTestSetup::Failed { error, .. }) = resources.loadtest_setup.take() {
                    input.error = Some(error);
                }
                Action::None
            }
            Mode::Starting { .. } => {
                // If loadtest is now active, transition to dashboard
                if resources.loadtest.is_some() {
                    self.mode = Self::new_dashboard();
                    return Action::None;
                }
                // If setup failed, go back to input with error
                if let Some(LoadTestSetup::Failed { error, .. }) = &resources.loadtest_setup {
                    let error = error.clone();
                    resources.loadtest_setup = None;
            ...
            Mode::Dashboard { table_state, auto_scroll } => {
                if *auto_scroll && !resources.flash.entries.is_empty() {
                    table_state.select(Some(0));
                }
                Action::None
            }
        ...
        match &mut self.mode {
            Mode::Input(input) => {
                if input.handle_key(key) {
                    let path = PathBuf::from(shellexpand::tilde(&input.value).as_ref());
                    if !path.exists() {
                        input.error = Some(format!("File not found: {}", path.display()));
                        return Action::None;
                    }
                    return Action::StartLoadTest(path);
                }
                Action::None
            }
            Mode::Starting { .. } => Action::None,
            Mode::Dashboard { table_state, auto_scroll } => {
                handle_dashboard_key(key, table_state, auto_scroll, _resources)
            }
        }
    }
    ...
    fn render(&mut self, frame: &mut Frame, area: Rect, resources: &Resources) {
        match &mut self.mode {
            ...
            Mode::Starting { config_path } => {
                render_starting(frame, area, config_path);
            }
            Mode::Dashboard { table_state, auto_scroll: _ } => {
                render_dashboard(frame, area, resources, table_state);
            }
        }
    }
}
...
fn render_input(frame: &mut Frame, area: Rect, input: &TextInput) {
    ...
    let title = Paragraph::new(Line::from(vec![
        ...
        Span::raw("  Enter path to gobrr config file"),
    ]));
...
fn render_starting(frame: &mut Frame, area: Rect, config_path: &Path) {
    ...
    let lines = vec![
        ...
        Line::from(""),
        Line::from(Span::styled(
            "Funding accounts and setting up test pipeline...",
...
fn render_metrics(f: &mut Frame, area: Rect, resources: &Resources) {
    ...
    // Inclusion Times
    lines.push(Line::from(Span::styled("INCLUSION TIMES", section_style)));
    if stats.inclusion_count > 0 {
        lines.push(Line::from(vec![
            Span::styled("  P50  ", label_style),
            Span::styled(format_ms(stats.percentile(50.0)), value_style),
            Span::styled("    P95  ", label_style),
            Span::styled(format_ms(stats.percentile(95.0)), value_style),
        ]));
        lines.push(Line::from(vec![
            Span::styled("  P99  ", label_style),
            Span::styled(format_ms(stats.percentile(99.0)), value_style),
        ]));
    } else {
