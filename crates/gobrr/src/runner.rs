...
pub async fn start_load_test(config_path: &str) -> Result<LoadTestHandle> {
    ...
    // Step 1c: Fetch chain ID from RPC
    ...
    // Step 2b: Clear mempools on management hosts if configured
    if !config.management_hosts.is_empty() {
        clear_mempools(&http_client, &config.management_hosts, &sender_addresses).await;
    }

    // Step 3: Run funding phase using Funder struct
    info!("Starting funding phase");
    let funder = Funder::new(http_client.clone(), &config.rpc, funder_signer, chain_id)
        .await
    ...
    // Step 5d: Set up rate limiter if target_tps is configured
    ...
    let rate_limiter_handle =
        if let Some((tps, limiter)) = config.target_tps.zip(rate_limiter.clone()) {
            ...
            let handle = tokio::spawn(async move {
                // Tick 10x per second, adding tps/10 permits per tick
                let permits_per_tick = ((tps / 10).max(1)) as usize;
                    ...
                    tokio::select! {
                        ...
                        _ = interval.tick() => {
                            ...
                            if available < max_permits {
                                let to_add = permits_per_tick.min(max_permits - available);
    ...
    // Step 6: Spawn signer and sender tasks for each sender
    ...
    for (i, signer) in sender_signers.into_iter().enumerate() {
        ...
        // Spawn signer task (generates + signs transactions)
        ...
        let signer_handle = tokio::spawn(async move {
            match Signer::new(client, &rpc, signer, sender_id, limiter, selector).await {
                Ok(s) => {
                    if let Err(e) = s.run(resign_rx, signed_tx, signer_block_rx, shutdown_rx).await
                    {
                        tracing::error!(sender = i, error = %e, "Signer failed");
                    }
                }
                Err(e) => {
                    tracing::error!(sender = i, error = %e, "Failed to create signer");
                }
            }
        });
        ...
        let sender_handle = tokio::spawn(async move {
            match Sender::new(client, sender_id, &rpc, in_flight, tracker, confirmer, resign_tx) {
                Ok(sender) => {
