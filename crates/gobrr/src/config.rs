...
impl fmt::Display for TxType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::LargeCalldata => write!(f, "large_calldata"),
        }
    }
}

/// Calldata size for simple ETH-send transactions
const SMALL_CALLDATA_SIZE: usize = 20;

/// Gas limit for simple / small-calldata transactions (21000 base + small buffer)
const GAS_LIMIT_SIMPLE: u64 = 25_000;

/// Computes gas limit for arbitrary calldata: 21000 base + 16 per byte + 20% margin
const fn compute_gas_limit(calldata_len: usize) -> u64 {
    let base: u64 = 21_000 + (calldata_len as u64) * 16;
    base + base / 5 // +20%
}
...
pub(crate) enum TxKind {
    ...
    EthSend,
    #[serde(rename = "large_calldata")]
    LargeCalldata { max_size: usize },
}
    ...
    pub(crate) fn build(&self, rng: &mut impl Rng) -> TxParams {
        match self {
            Self::EthSend => TxParams {
                to: Address::ZERO,
                value: U256::ZERO,
                input: generate_calldata(rng, SMALL_CALLDATA_SIZE),
                gas_limit: GAS_LIMIT_SIMPLE,
                tx_type: TxType::EthSend,
            },
            Self::LargeCalldata { max_size } => {
