...
impl Signer {
    /// Creates a new Signer by fetching chain state from the RPC
    pub(crate) async fn new(
        ...
        let max_fee_per_gas = base_fee * GAS_FEE_MULTIPLIER;
        ...
        loop {
            tokio::select! {
                ...
                result = block_rx.recv() => {
                    ...
                    if let Err(e) = self.handle_sign(unsigned, &signed_tx).await {
                        warn!(sender = self.sender_id, error = %e, "Failed to sign transaction");
                    }
                }
            }
        }
    ...
    async fn handle_sign(
        ...
        // Acquire rate-limiter permit if configured (blocks until available)
        if let Some(ref limiter) = self.rate_limiter {
            let _permit = limiter.acquire().await.context("Rate limiter closed")?;
        }
    ...
    async fn handle_resign(
        ...
        // EIP-1559 replacement requires bumped priority fee too
        self.max_priority_fee_per_gas = self.max_priority_fee_per_gas * 120 / 100;
        if self.max_priority_fee_per_gas == 0 {
            self.max_priority_fee_per_gas = 1_000_000; // minimum 1M wei
        }
