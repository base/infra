...
/// Skips any addresses that match `skip_address` (e.g., the funder address)
pub(crate) fn derive_signers(
    ...
    while signers.len() < count as usize {
        ...
        if Some(signer.address()) == skip_address {
            tracing::warn!(index, address = %signer.address(), "Skipping derived address that matches funder");
            index += 1;
            continue;
        }
