...
enum SendErrorKind {
    ...
    Transient,
    /// "nonce too low", "already known" — discard
    Stale,
}
...
impl Sender {
    ...
    async fn spawn_batch(&self, tasks: &mut JoinSet<()>, batch: Vec<SignedTx>) -> Result<()> {
        ...
        tasks.spawn(async move {
            send_batch(idx, batch, provider, tracker, confirmer, resign, errors).await;
            drop(permit);
        });
        Ok(())
    }
...
async fn send_batch(
    ...
    debug!(sender = sender_id, batch_size, "Sending transaction batch");
    ...
    for tx in &batch {
        match batch_req.add_call::<_, B256>("eth_sendRawTransaction", &(tx.raw_bytes.clone(),)) {
            ...
            Err(e) => {
                error!(sender = sender_id, tx_hash = %tx.tx_hash, error = ?e, "Failed to add tx to batch");
            }
        }
    }
    ...
    if let Err(e) = batch_req.send().await {
        error!(sender = sender_id, batch_size, error = ?e, "Failed to send batch request");
        // Whole-batch HTTP failure — count all txs as transient failures
        consecutive_errors.fetch_add(batch_size as u32, Ordering::Relaxed);
        for tx in batch {
    ...
    // Collect results
    for (tx, fut) in batch.into_iter().zip(futures) {
        match fut.await {
            Ok(_) => {
                ...
                match kind {
                    ...
                    SendErrorKind::Stale => {
                        debug!(
                            ...
                            nonce = tx.nonce,
                            tx_hash = %tx.tx_hash,
                            "Stale tx (nonce too low / already known)"
                        );
                        let _ = tracker_tx.send(TrackerEvent::TxFailed {
                            tx_hash: tx.tx_hash,
                            tx_type: tx.tx_type,
